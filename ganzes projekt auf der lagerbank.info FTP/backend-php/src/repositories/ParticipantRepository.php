...
class ParticipantRepository {
    ...
    public function getById($id) {
        $query = "
            ...
            LEFT JOIN registrations r ON p.registration_id = r.id
            LEFT JOIN tents t ON p.zelt_id = t.id
            LEFT JOIN auth_users u ON p.checked_in_by_id = u.id
            WHERE p.id = ?
        ";
    ...
    public function create($camp_id, $registration_id, $user_id = null) {
        $query = "
            ...
            VALUES (?, ?, ?, 'gesund', NOW())
        ";
        $this->db->execute($query, [$camp_id, $registration_id, $user_id]);
    ...
    public function update($id, $data) {
        ...
        $params[] = $id;
        $query = "UPDATE participants SET " . implode(", ", $updates) . " WHERE id = ?";
    ...
    public function checkIn($id, $checked_in_by_id) {
        $query = "
            ...
            WHERE id = ?
        ";
        return $this->db->execute($query, [$checked_in_by_id, $id]);
    }
    ...
    public function getTentOccupancy($tent_id) {
        ...
        $result = $this->db->execute($query, [$tent_id])->fetch();
